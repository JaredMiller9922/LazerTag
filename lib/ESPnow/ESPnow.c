...
/**
 * @brief Constructor for the IRReceiver Paring in this state the gun will send out its ir encoded mac, 
 * and also await to recieve data through ir that contains its own mac along with a vest mac then init espnow
 */
bool paring = true; 
...
// Define the peer MAC address (this will be the other mac address out of the gun vest pare)
 uint8_t peer_mac[6] = {0x24, 0x58, 0x7c, 0xd6, 0xc1, 0x44}; // black
...
/**
 * @brief Callback function to handle the status of sending data
 */
...
void espnow_init()
{
    ...
    ESP_LOGI(ESPNOW_TAG, "Peer added: " MACSTR, MAC2STR(peer_mac));
}
